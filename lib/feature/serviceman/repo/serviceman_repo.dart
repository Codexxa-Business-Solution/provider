...
class ServicemanRepo{
  ...
  Future<Response> getAllServicemanData(int offset,String status) async {
    return await apiClient.getData(AppConstants.SERVICEMAN_LIST_URI+"?limit=${Get.find<SplashController>().configModel.content!.paginationLimit}&offset=$offset&status=$status");
  }

  Future<Response> getServicemanDetails(String id) async {
    return await apiClient.getData(AppConstants.SERVICEMAN_DETAILS_URI+"/"+id);
  }
  Future<Response> assignServiceman(String bookingId,String servicemanId) async {
    return await apiClient.postData(AppConstants.SERVICEMAN_ASSIGN_URI+"/"+bookingId,{'serviceman_id':servicemanId,'_method':'put'});
  }

  Future<Response> changeServicemanStatus(String servicemanId) async {
    return await apiClient.putData(AppConstants.SERVICEMAN_CHANGE_STATUS+"?serviceman_id[]=$servicemanId",{});
  }


  Future<Response> getSingleServicemanData(String id) async {
    return await apiClient.getData(AppConstants.SERVICEMAN_LIST_URI+"/"+id);
  }

  Future<Response> deleteServiceman(String id) async {
    return await apiClient.deleteData(AppConstants.SERVICEMAN_DELETE_URI+"?serviceman_id[]="+id);
  }
  ...
  Future<Response?> addNewServiceman(ServicemanBody servicemanBody,XFile? profileImage, List<MultipartBody> identityImage) async {
    ...
    return await apiClient.postMultipartData(AppConstants.NEW_SERVICEMAN_Add_URI, _body,
  ...
  Future<Response?> editServicemanInfoWithPassword(ServicemanBody servicemanBody,XFile? profileImage, XFile? identityImage,String servicemanId) async {
    ...
    return await apiClient.postMultipartData(AppConstants.NEW_SERVICEMAN_Add_URI+"/"+servicemanId, _body,
